...
static void init_parse_table(void) {
    ...
    for (int row = 0; row < PARSE_TABLE_ROWS; row++) {
        nonterm_sym = index_to_nonterminal(row);

        for (int col = 0; col < PARSE_TABLE_COLS; col++) {
            switch (nonterm_sym) {
                case STMT:
                    switch (col) {
                        case NUM:
...
static void init_todo_table(void) {
    for (int i = 0; i < TODO_TABLE_ROW_LEN; i++) {
        for (int j = 0; j < TODO_TABLE_COL_LEN; j++) {
            switch (i) {
                case 0: // stmt
                    switch (j) {
                        case 0:
...
static int parse_stack_pop(void) {
    ...
    int result = parse_stack[parse_stack_index];
...
static int value_stack_pop(void) {
    ...
    int result = value_stack[value_stack_index];
